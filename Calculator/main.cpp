...
const HashMap<char, Evaluator<Double>::UnaryOperator> getUOps()
{
	...
	return HashMap<char, Evaluator<Double>::UnaryOperator>(names, uOps, 2);
}
...
const HashMap<char, Evaluator<Double>::BinaryOperator> getBOps()
{
	...
	return HashMap<char, Evaluator<Double>::BinaryOperator>(names, bOps, 2);
}
...
const HashMap<string, Evaluator<Double>::Function> getFunctions()
{
	...
	return HashMap<string, Evaluator<Double>::Function>(names, funcs, 11);
}
...
#pragma region Functions
	...
	{
		return sin(!rad?Evaluator<Double>::toRadians(x):x);
	}
	const Double cosine(const Double x, const bool rad)
	{
		return cos(!rad?Evaluator<Double>::toRadians(x):x);
	}
	const Double tg(const Double x, const bool rad)
	{
		return tan(!rad?Evaluator<Double>::toRadians(x):x);
	}
	const Double ctg(const Double x, const bool rad)
	{
		return 1./tan(!rad?Evaluator<Double>::toRadians(x):x);
	}
	const Double asine(const Double x, const bool rad)
	{
		const double res(asin(x));
		return (!rad?Evaluator<Double>::toDegrees(res):res);
	}
